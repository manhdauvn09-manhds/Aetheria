...
| 4 | Coding (sub-schedule below)                | 🟨     | 79 | 4.1–4.47 done; Leaderboards — Redis ZSET `aetheria:lb:{mode}:{seasonId}` + `lbTop`/`lbAroundMe` queries + auto-record in `MmrService.applyMatchResult` |
| 5 | Code review                                | ⬜     | 0  |                                                  |
