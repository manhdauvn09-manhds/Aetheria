...
48. **NEXT — Step 4.45**: Realtime PvP turns — apps/realtime subscribes to `aetheria:pvp:match.start`, auto-joins players to `match:<id>`, accepts `pvp:action` events (validated via `domain-combat`), broadcasts state updates, fires forfeit on heartbeat-miss. Vertical L.
