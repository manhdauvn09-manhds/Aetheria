...
50. **NEXT — Step 4.47**: Leaderboards — `lb.recordResult` → Redis ZSET `lb:{mode}:{seasonId}`; `lb.top/aroundUser` queries; snapshot the `leaderboards` MySQL table on a worker tick. Vertical M, deps 4.46.
