...
44. **NEXT — Step 4.41**: Realtime chat channels — wire ChatService.send to publish `chat:message` to `channel:<type>:<id>` rooms (or `user:<id>` for whisper) via `apps/realtime`'s Socket.IO server. Will require a small in-process bridge: api emits → redis pub → realtime adapter routes to room subscribers.
