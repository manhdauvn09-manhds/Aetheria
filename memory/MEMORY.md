...
36. **NEXT — Step 4.34**: Account-level XP grant flow + `progression.applyXp` runtime helper (extracts the inline xp-claim logic from QuestService into a shared service so combat / battle-pass / level-rewards can grant XP without duplicating the levelFromTotalXp + addXp + emit pipeline). Vertical S, dep 4.33.
