...
41. **NEXT — Step 4.38**: Friends service (list / request / respond) — vertical M, deps 4.7. Should follow the same `Pick<MysqlClient>` + service + router + audit + rules pattern, using existing `Friendship` model (status: pending|accepted|blocked).
