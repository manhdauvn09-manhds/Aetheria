...
49. **NEXT — Step 4.46**: MMR (Glicko-2) — `mmr.get/update`; post-match write to `mmr` table + `pvpMatchPlayer.mmrAfter`. Vertical M, deps 4.45. Will hook into `MatchService.complete` to compute deltas before persisting `result`.
