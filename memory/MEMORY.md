...
42. **Step 4.39 — Chat service (send / history / report; flagged audit)** (5 May 2026):
    - Operates on existing `chat_messages` model. Channel semantics: `global` (channelId null), `guild` (= Guild.id, member-only), `party` (transient BigInt, no membership table), `whisper` (= other user's id, blocks enforced both directions).
    - **`packages/domain-social/src/chat/`** subpackage: `types.ts` (`ChatMessageRow`, per-op inputs), `service.ts` (`ChatService` with `Pick<MysqlClient, "chatMessage" | "guildMember" | "friendship">` + injectable clock + optional profanityList override), `router.ts` (`createChatRouter(svc)` exposing `send`, `history`, `report`).
    - **`send` pipeline**: (1) `validateContent` from 4.36 (rejects empty / long / multi-line / control-chars); (2) `assertCanWrite` — guild-member check for guild, block-check both directions for whisper, no check for global/party, self-whisper rejected; (3) sliding-window `checkRateLimit(history, now, DEFAULT_RATE_LIMITS[channelType])` from a `findMany` of recent same-channel messages; (4) repeat-spam count over the last 60s (`recentSameContentCount`) → `shouldFlagMessage` → `flagged` column; (5) insert `ChatMessage` row; (6) audit-log `chat.flagged` only when flagged (every-send audit would be too noisy).
    - **`history`**: protected, default 50, max 200. Whisper history pairs `(actor, other)` regardless of who spoke — `OR: [{ sender:actor, channel:other }, { sender:other, channel:actor }]`. Guild + whisper enforce read-side access via `assertCanRead`. Optional `before: Date` for cursor-style paging.
    - **`report`**: marks `flagged=true` (idempotent — only updates if not already flagged) + writes audit log `chat.report` with the reporter's reason (1–500 chars).
    - **Errors**: `badRequest` (validation, missing channelId, self-whisper, bad reason), `forbidden` (non-member, blocked), `conflict` (rate-limit with `retryAfterMs` payload), `notFound` (missing message). All wrapped via `asTrpcError` in router.
    - **`exactOptionalPropertyTypes` workarounds**: keep optional service-state fields as explicit `| undefined` (`profanityList: readonly string[] | undefined`); pass to helpers via conditional spread `...(this.profanityList ? { profanityList: this.profanityList } : {})`. Same trick in router for forwarding `limit`/`before`. Avoids `!` vs `as` lint conflict by re-narrowing nullable channelId with explicit `if (channelId === null) throw …` before calls.
    - **Wiring**: `apps/api/src/server.ts` adds `new ChatService({ mysql })`. `apps/api/src/router.ts` mounts `chat: createChatRouter(deps.chatService)`.
    - **Smoke** (5 May 2026): repo-wide `pnpm -r typecheck` 22/22 ✓, `pnpm -r lint` 22/22 ✓, `pnpm -r test` **287/287 ✓** (no new tests this slice — moderation rules already covered in 4.36b).
43. **Step 4.40 — `apps/realtime` boot (Socket.IO + Redis adapter + sticky helper)** (5 May 2026):
    - New workspace package `@aetheria/realtime` (apps/realtime). Port `REALTIME_PORT=3002` default. Reuses `JWT_SECRET` / `JWT_ISSUER` / `JWT_AUDIENCE` from apps/api so the same access tokens authenticate WS handshakes.
    - **`src/env.ts`** — zod-validated env: `NODE_ENV`, `REALTIME_HOST/PORT`, `CORS_ORIGIN` (csv→string[]), `JWT_*` (secret ≥32), `REDIS_URL` optional, `REALTIME_INSTANCES` (1–64, default 1).
    - **`src/sticky.ts`** (pure) — `fnv1a32(str)` 32-bit FNV-1a (`Math.imul` for 32-bit-safe mul, `>>>0` to unsigned), `shardForUser(userId, instances)` returns 0 when `instances ≤ 1` else `fnv1a32(userId.toString()) % instances`, `STICKY_COOKIE_NAME = "aetheria_rt_shard"` for sticky-session-aware LBs.
    - **`src/auth.ts`** — `buildAuthMiddleware(cfg)` returns a non-async `(socket, next) => void` middleware that calls `jwtVerify(token, key, { issuer, audience, algorithms: ["HS256"] })` and assigns `socket.auth = { userId: sub, roles }`. Token source: `socket.handshake.auth.token` first, then `authorization: Bearer …` header (no query-string fallback — query strings leak into proxy logs). `socket.io` module augmented to add `socket.auth?: SocketAuth`. **Key lint trick**: avoid `no-misused-promises` by writing the middleware as non-async with explicit `.then().catch()` instead of `async fn`.
    - **`src/server.ts`** — `buildRealtime(env)` (sync, returns `{ http, io, log, close }`). Raw `node:http` server handles `/healthz` and `/sticky-cookie?u=<userId>` (sets the LB sticky cookie based on `shardForUser`). Socket.IO attached with `cors`, `serveClient: false`, `transports: ["websocket","polling"]`. When `REDIS_URL` is set, `pubClient.duplicate()` + `createAdapter(pub, sub)` gives cross-instance pub/sub; otherwise warns "single-instance" at boot. On `connection` the socket joins a `user:<userId>` room (chat fan-out 4.41 will publish to those rooms).
    - **`src/index.ts`** — boot wrapper: SIGINT/SIGTERM run `rt.close()` (drains io → quits redis → closes http) then `process.exit`. Listens via `rt.http.listen(...)` callback wrapped in a Promise.
    - **Tests** — 7 vitest specs in `__tests__/sticky.test.ts` (FNV-1a known outputs incl. empty + "a", determinism, distinct inputs differ; `shardForUser` trivial single-instance, range invariant over 50 ids, string-vs-bigint stability; sticky cookie name non-empty).
    - **Deps added**: `socket.io`, `@socket.io/redis-adapter`, `ioredis`, `pino`, `pino-pretty`, `jose`, `zod` (+ tsx/vitest dev). pnpm-lock.yaml regenerated.
    - **Smoke** (5 May 2026): `pnpm --filter @aetheria/realtime typecheck` ✓, `lint` ✓, `test` 7/7 ✓. Repo-wide `pnpm -r typecheck` 23/23 ✓, `pnpm -r lint` 23/23 ✓, `pnpm -r test` **294/294 ✓** (was 287; +7 from sticky).
44. **Step 4.41 — Realtime chat channels (Socket.IO rooms via redis pub/sub bridge)** (5 May 2026):
    - **Bridge**: `apps/api` ChatService publishes each accepted send to redis channel `aetheria:chat:bus`; `apps/realtime` subscribes (separate Redis duplicate connection from the Socket.IO adapter) and re-emits as `chat:message` to the matching rooms. Fan-out is opt-in: when `REDIS_URL` is unset, ChatService persists fine and clients see new messages on their next history poll.
    - **`packages/domain-social/src/chat/publisher.ts`** — exports `REDIS_CHAT_CHANNEL = "aetheria:chat:bus"`, `ChatBusMessage` (BigInts stringified, `createdAt` ISO), `RealtimePublisher` interface, `toBusMessage(row)`, and `redisChatPublisher(redis, log?)` factory that swallows publish errors so realtime hiccups never break the durable write path. `ChatService` now takes optional `publisher` dep and calls it after persist (step 7 in the send pipeline).
    - **`apps/realtime/src/rooms.ts`** (pure) — `channelRoom(type, id)` returns `"channel:global"` / `"channel:guild:<id>"` / `"channel:party:<id>"` or null for whisper; `userRoomsForWhisper(senderId, channelId)` returns `["user:<sender>", "user:<channel>"]`; `broadcastTargets({channelType, channelId, senderId})` resolves to the 1-or-2 rooms a single bus message should hit.
    - **`apps/realtime/src/chatBus.ts`** — `attachChatBus(io, subClient, log)`: subscribes to `REDIS_CHAT_CHANNEL`, validates payload shape (string-typed + literal-union channelType + null|string channelId + boolean flagged), drops malformed messages with a warn-log, and `io.to([...rooms]).emit("chat:message", msg)`.
    - **`apps/realtime/src/server.ts`** — third Redis connection (`pubClient.duplicate()`) for the chat bus subscriber, kept separate from the io.adapter pair so unsubscribe can run cleanly. On `connection`, sockets auto-join `user:<userId>` + `channel:global`; clients can `chat:join` / `chat:leave` for guild/party rooms with payload `{channelType, channelId}` (channelId must be `\d+`). Whisper rooms are never explicit-joined; they're derived from `user:<id>`.
    - **Wiring**: `apps/api/src/server.ts` builds a Redis client from `REDIS_URL` and passes `redisChatPublisher(redis, { warn })` into `new ChatService({ mysql, ...(pub ? { publisher: pub } : {}) })` (conditional spread to satisfy `exactOptionalPropertyTypes`). `chatRedis.quit()` added to the `onClose` hook. `apps/realtime/package.json` adds `@aetheria/domain-social` workspace dep so it can import the bus channel constant + types.
    - **Security note**: realtime trusts client `chat:join` claims; durable enforcement still happens at api-side `assertCanWrite/Read`. Clients that join a guild they aren't in will receive broadcasts but cannot post (api rejects). Tightening to membership-validated joins is a follow-up if needed.
    - **Tests** — 10 vitest specs in `apps/realtime/src/__tests__/rooms.test.ts` (channelRoom for each type + null cases; userRoomsForWhisper present + empty; broadcastTargets all 4 channel types + missing channelId).
    - **Smoke** (5 May 2026): repo-wide `pnpm -r typecheck` 23/23 ✓, `pnpm -r lint` 23/23 ✓, `pnpm -r test` **304/304 ✓** (was 294; +10 from rooms).
45. **Step 4.42 — Web Guild / Friends / Chat (closes Phase 4-F)** (5 May 2026):
    - Added `guild.myMembership` query to `domain-social/guild` (service + router) returning `{ guildId, role } | null` so web can branch between create-guild and in-guild views without the user pasting an id.
    - **`/guild` (`apps/web/src/app/guild/page.tsx`)** — branches on `myMembership`: not-in-guild renders create-form (name 3–64, tag 2–4, optional description) + pending-invites list with accept/decline; in-guild renders header card (name, tag, level, xp, member count, viewer's role) + officer/leader-only Invite form + member list with role pills + leader-only `<select>` to change role + kick button gated by `roleRank` mirror of server-side rules.
    - **`/friends` (`apps/web/src/app/friends/page.tsx`)** — Add-friend by user-id form + 4 buckets (Friends / Incoming / Outgoing / Blocked). Incoming rows expose Accept + Decline buttons. Server already hides blocks-against-the-viewer.
    - **`/chat` (`apps/web/src/app/chat/page.tsx`)** — Tabs: Global / Guild / Whisper. Guild + Whisper require an id input. Seeds `messages` from `trpc.chat.history` (newest-first → reversed to chronological), live-appends from Socket.IO `chat:message` events filtered by `matchesActive(msg, active, viewer)`. Optimistic on send: appends only if the realtime echo hasn't already delivered the same `messageId`.
    - **`apps/web/src/lib/realtime/useChatSocket.ts`** — hook: connects to `NEXT_PUBLIC_REALTIME_URL` with `auth: { token: access.value }` (Zustand session), websocket-only, joins `user:<id>` + `channel:global` automatically (server-side), emits `chat:join` / `chat:leave` for guild/party, exposes `{ connected, messages, replace, append }`. Whisper requires no explicit join (uses server-side `user:<id>` rooms).
    - **`apps/web/src/lib/env.ts`**: added `NEXT_PUBLIC_REALTIME_URL` (default `http://localhost:3002`).
...
47. **Step 4.44 — PvP match lifecycle (`pvp_matches` row on found + room hint + `pvp.match` query)** (5 May 2026):
    - Adds `PvpMatchService` (deps `Pick<MysqlClient, "pvpMatch" | "pvpMatchPlayer" | "mmr" | "$transaction">`, optional `MatchPublisher`, optional `seedFactory`) to `@aetheria/domain-pvp`.
    - **`createFromProposal(proposal)`** atomic `$transaction`: INSERTs one `pvp_matches` row (`status="active"`, `region`, `mode`, `serverSeed = randomBytes(64)`) + two `pvp_match_players` rows (`mmrBefore = entry.mmr` snapshotted, `mmrAfter = mmrBefore` placeholder, `lineup = []`, `score = 0`, `result = "draw"`). Audit `pvp.match.start` (actor=a, opponent payload, waitedMs). Then publishes a `MatchStartEvent` `{ matchId, mode, region, players, startedAt }` via the optional Redis publisher.
    - **`get(matchId, requesterUserId)`** — full row including parsed lineup; rejects with `forbidden` when the requester isn't a participant.
    - **`activeMatchFor(userId)`** — finds the participant's most-recent `active` match (`pvpMatchPlayer.findFirst({ where: { userId, match: { status: "active" } }, orderBy: { matchId: "desc" } })`).
    - **`publisher.ts`** — `REDIS_PVP_MATCH_CHANNEL = "aetheria:pvp:match.start"`; `toMatchStartEvent(row)` (BigInts→strings, dates ISO); `redisMatchPublisher(redis, log?)` swallows publish errors so a flaky bus never breaks the durable write path.
    - **Router**: `createPvpRouter(matchmaking, matches)` now also exposes `match({ matchId })` + `activeMatch` (both `protectedProcedure`).
    - **Wiring**: `apps/api/src/server.ts` builds `redisMatchPublisher` from `pvpRedis`, instantiates `PvpMatchService({ mysql, publisher? })`, and uses it as `pvpService`'s `onMatch` callback (`createFromProposal(proposal)` chained with `.then/catch` so a failed write logs but doesn't crash the matcher loop). Threaded into `AppDeps.pvpMatchService`.
    - **Realtime room creation**: deferred to a follow-up (4.45) — apps/realtime will subscribe to `aetheria:pvp:match.start` and auto-join both players to a `match:<id>` room.
    - **Tests** — 3 vitest specs in `__tests__/match.test.ts` (REDIS channel name, BigInt/Date stringification in `toMatchStartEvent`, player-order preservation).
    - **Smoke** (5 May 2026): repo-wide `pnpm -r typecheck` 24/24 ✓, `pnpm -r lint` 24/24 ✓, `pnpm -r test` **323/323 ✓** (was 320; +3 from match).
48. **Step 4.45 — Realtime PvP turns (action loop + heartbeat forfeit + match.end bridge)** (5 May 2026):
    - **`apps/realtime/src/pvp.ts`** (pure): `parseMatchStart`/`parsePvpAction` (strict shape validators), `buildInitialState`, `evaluateAction(state, actor, action, now)` returning `{ok, nextState, ended?}` with reasons `not_participant`/`not_your_turn`/`match_ended`/`match_id_mismatch`. Action kinds: `move|ability|surrender|heartbeat`. Heartbeat is a no-op pass; surrender ends with opponent winner; move/ability alternates `turnUserId` and bumps `turnNumber`; `MAX_TURNS=30` → draw on completion; `TURN_TIMEOUT_MS=30_000`. `onDeadlineMiss(state)` returns `{winnerUserId, loserUserId}`.
    - **`apps/realtime/src/pvpMatches.ts`** — subscribes to `aetheria:pvp:match.start` via a dedicated Redis duplicate. On match-start: builds in-memory `MatchState` (Map keyed by matchId), uses `io.in(\`user:<id>\`).socketsJoin(\`match:<id>\`)` to push both players into the per-match room, emits `pvp:start`, arms a `setTimeout` deadline. On `pvp:action` socket event: parses, runs `evaluateAction`, emits `pvp:state` to room, ack-replies `{ok}` to caller. On end (surrender/timeout/MAX_TURNS): clears timer, emits `pvp:end`, publishes `aetheria:pvp:match.end` (separate Redis pub client). On deadline miss: forfeits current turn-holder via the same end channel.
    - **`apps/realtime/src/server.ts`**: 5 Redis duplicates total now (pub for adapter, sub for adapter, chatBus sub, pvp.start sub, pvp.end pub) all derived from a single `pubClient = new Redis(env.REDIS_URL)` via `.duplicate()`. Connection handler delegates `pvp:action` to `attachPvpMatches`. Close path tears down `pvpMatches.close()` before `pvpEndPub.quit()`.
    - **`packages/domain-pvp/src/match.ts`** adds `complete(matchId, winnerUserId, reason="completed"|"forfeit"|"timeout")` (atomic `$transaction` updating `pvp_matches.status` + per-player `result`; status becomes `abandoned` on forfeit, otherwise `completed`; `mmrAfter` deferred to 4.46) and `forfeit(matchId, forfeiterUserId)` shorthand. Audit `pvp.match.<reason>`.
    - **`packages/domain-pvp/src/publisher.ts`**: new `REDIS_PVP_MATCH_END_CHANNEL = "aetheria:pvp:match.end"`, `MatchEndEvent`/`MatchEndPublisher`, `redisMatchEndPublisher` factory.
    - **`apps/api/src/pvp/matchEndConsumer.ts`** subscribes to the end channel, validates payload shape, calls `service.forfeit` for `reason==="forfeit"` else `service.complete(...)`. Errors swallowed via `.catch` so a single bad message never kills the consumer. Wired in `apps/api/src/server.ts` via `pvpRedis.duplicate()`; closed in onClose.
    - **Combat-domain validation**: deferred — current `evaluateAction` only enforces turn ownership + match membership. Plugging `applyAction(state, action)` from `domain-combat` is left as a follow-up; the action payload is broadcast as-is so clients can reconstruct.
    - **Tests** — 14 vitest specs in `apps/realtime/src/__tests__/pvp.test.ts` (parseMatchStart accept/reject incl. bad mode/region/types/null; parsePvpAction accept/reject for all kinds + bad payload; buildInitialState seeds correctly; evaluateAction non-participant/not-your-turn/matchId-mismatch/normal-alternation/heartbeat-noop/surrender/MAX_TURNS-draw/ended-rejection; onDeadlineMiss forfeits turn-holder).
    - **Smoke** (5 May 2026): repo-wide `pnpm -r typecheck` 24/24 ✓, `pnpm -r lint` 24/24 ✓, `pnpm -r test` **337/337 ✓** (was 323; +14 from pvp realtime helpers).
49. **Step 4.46 — MMR Glicko-2 (pure + service + post-match write)** (5 May 2026):
    - **Schema migration `0003_mmr_glicko`**: `mmr` table gains `rd DOUBLE NOT NULL DEFAULT 350.0` + `volatility DOUBLE NOT NULL DEFAULT 0.06`. Mirrored in `db/mysql/01_init.sql`. Prisma client regenerated via `pnpm --filter @aetheria/schema-db generate:mysql`.
    - **`packages/domain-pvp/src/glicko.ts`** (pure): canonical Glickman 2013 equations — `glicko2Update(self, opponents, τ=0.5)` + `glicko2Single(self, opponent, score)` shorthand. `SCALE=173.7178`, `ANCHOR=1500`. Volatility solver uses Illinois variant of regula-falsi (`MAX_ITERATIONS=50`, `CONVERGENCE=1e-6`). No-games branch only inflates RD by `√(φ²+σ²)`. `DEFAULT_RATING = { 1000, 350, 0.06 }`.
    - **`packages/domain-pvp/src/mmr.ts`** — `MmrService(Pick<MysqlClient,"mmr"|"pvpMatchPlayer"|"$transaction">)`. `get(userId, mode)` returns the row or a virtual default snapshot when missing. `applyMatchResult({matchId, mode, playerA, playerB, winnerUserId})`: reads both ratings, runs `glicko2Single` per side (`winnerUserId === null` → 0.5/0.5 draw), upserts `mmr` (+peakMmr/seasonGames/seasonWins) and updates `pvpMatchPlayer.mmrAfter` for both rows in one `$transaction`. Returns per-player `MmrDelta` for downstream broadcasting. Audit `pvp.mmr.update`.
    - **`MatchService` hook**: `MatchDeps.mmrService?` added; `complete()` calls `mmrService.applyMatchResult(...)` after the status update for **1v1 with exactly 2 players**. Errors bubble — MMR is part of the match contract, not optional. 3v3 path is a follow-up.
    - **Router**: `createPvpRouter(matchmaking, matches, mmr)` now takes a third arg; new `pvp.mmr({mode})` query returns the snapshot for the calling user.
    - **Wiring** (`apps/api/src/server.ts`): `new MmrService({ mysql })` constructed once, passed into `PvpMatchService` deps and threaded into the app router via `AppDeps.mmrService`.
    - **Tests** — 5 vitest specs in `__tests__/glicko.test.ts`: Glickman 2013 worked example matches r≈1464/RD≈151/σ≈0.06 within 1 unit; equal-opponent win/loss moves rating in expected directions; draw vs equal opponent ≤2 rating delta; RD shrinks after a game; no-games path only inflates RD.
    - **Smoke** (5 May 2026): repo-wide `pnpm -r typecheck` 24/24 ✓, `pnpm -r lint` 24/24 ✓, `pnpm -r test` **342/342 ✓** (was 337; +5 from glicko).
50. **Step 4.47 — PvP Leaderboards (Redis ZSET + lbTop/lbAroundMe)** (5 May 2026):
    - Live snapshot lives in Redis: key `aetheria:lb:{mode}:{seasonId}`, score = current MMR (rating, integer), member = userId stringified. MySQL `leaderboards` snapshot table is owned by a worker (Phase 4-I 4.55) — not touched here.
    - **`packages/domain-pvp/src/leaderboard.ts`** — `LeaderboardService(Pick<Redis, "zadd"|"zrevrange"|"zrevrank"|"zcard"|"zscore">)` with optional `seasonId` default. Pure helpers `lbKey(mode, seasonId)` + `slidingWindow(rank, radius, total)` (clamps low/high, treats negative radius as zero, returns `{0,-1}` on empty set). Service methods: `recordResult({userId, mode, mmr, seasonId?})` (idempotent ZADD), `top(mode, limit=50, seasonId?)` (capped at 500), `aroundUser(userId, mode, radius=5, seasonId?)` returning `{rank, mmr, entries}` window via `zrevrank` + `zcard` + `zrevrange`.
    - **MmrService hook**: `MmrDeps.leaderboard?` added; `applyMatchResult` calls `Promise.all([recordResult(a), recordResult(b)])` after the durable transaction commits (so a leaderboard hiccup never rolls back the rating). Skipped when leaderboard dep is absent.
    - **Router**: `createPvpRouter(matchmaking, matches, mmr, lb)` now takes a fourth arg; new `pvp.lbTop({mode, limit?, seasonId?})` + `pvp.lbAroundMe({mode, radius?, seasonId?})` queries (both `protectedProcedure`; bounds `limit ∈ [1,500]`, `radius ∈ [0,50]`).
...
58. **Steps 4.58–4.63 — Phase 4-J (frontend polish / PWA / i18n / telemetry)** (5 May 2026):
    - **4.58 Scene state machine**: existing `useGameFlow` Zustand store already covered the spec; added `apps/web/src/lib/game/sceneTransitions.ts` (pure `canTransition` / `assertCanTransition` over a 6-node adjacency graph: splash → main_menu → realm_picker / level_picker → in_game ↔ pause → main_menu).
    - **4.59 PWA**: `apps/web/public/manifest.webmanifest` + `apps/web/public/sw.js` (install precaches `/`, `/menu`, `/manifest.webmanifest`; activate prunes old caches; fetch is stale-while-revalidate for same-origin GET; `/trpc` and `/api/` always passthrough). Registered via `components/pwa/SwRegister.tsx` mounted in root layout (production-only `navigator.serviceWorker.register`). `metadata.manifest` + `themeColor` set in layout. IndexedDB asset cache deferred.
    - **4.60 i18n**: `apps/web/src/lib/i18n/messages.ts` ships `en` + `vi` bundles with namespace shape (`app`, `menu`, `pvp`); `Bundle = { [N]: { [K]: string } }` widened from `typeof en` so locales aren't forced to copy literal values. `lib/i18n/i18n.ts` exports `useI18n` (Zustand + `persist` to `localStorage` key `aetheria.lang`) and `useT()` returning `{ t, locale }`. Lookup walks namespace `.` leaf, falls back to `en`, then literal key.
    - **4.61 Telemetry**: new package `@aetheria/domain-telemetry` — service stores events as `audit_log` rows (`action = "telemetry.<event>"`) so no new table. Funnel allowlist (`KNOWN_EVENTS`: signup_complete, tutorial_complete, level_clear, account_level_5/_25, shop_purchase, pvp_first_match, guild_join, page_view, error). Batch capped at 50; payload validator `isPayloadShallow` rejects nested objects/arrays. Router exposes `telemetry.event` as **`publicProcedure`** (signup-funnel events fire pre-login) but reads `ctx.auth?.userId` when available. **`exactOptionalPropertyTypes` quirk**: `TelemetryEvent` fields use explicit `| undefined` to satisfy the strict optional check from zod's `optional()`. 7 vitest specs.
    - **4.62 OpenTelemetry**: `packages/core/src/telemetry/tracing.ts` — `startTracing({serviceName, endpoint?})` returns `{serviceName, enabled, shutdown()}`. Currently a **no-op scaffold** (no `@opentelemetry/sdk-node` install) — `enabled = Boolean(endpoint)`. Production swaps the impl behind `OTEL_EXPORTER_OTLP_ENDPOINT` without touching call sites.
    - **4.63 Sentry**: `packages/core/src/telemetry/sentry.ts` — `startSentry({dsn?, environment?})` returns `{enabled, captureException(err), flush()}`. **No-op scaffold** until `SENTRY_DSN` set; same swap-impl pattern.
